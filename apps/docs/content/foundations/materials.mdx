...
Solid material provides better contrast and readability, making it ideal for accessibility-focused designs. Translucent material can reduce contrast, especially on complex backgrounds. Text reads the strong ink by default, so keep it that way on translucent surfaces rather than dropping to a quieter `emphasis`.

```tsx
<Button material="translucent">Accessible button</Button>
```
