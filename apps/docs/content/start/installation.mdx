...
<Callout.Root variant="soft" size="2" color="gray">
  <Callout.Icon>
...
`styles.css` loads no webfonts — it defaults to system stacks, so installing Kookie costs zero font requests. Choose one of:

```tsx
// Kookie's own typefaces: Inter, JetBrains Mono, Playfair Display.
// Three requests to Google Fonts.
import '@kushagradhawan/kookie-ui/fonts.css';
```

```css
/* Or bring your own, from next/font, a self-hosted @font-face, anywhere. */
:root {
...
See [Typography](/foundations/typography) for the full font token model, including how to give headings their own typeface.
  ...
  return (
    <Card size="3" variant="soft">
      <Flex direction="column" gap="3" p="4">
        ...
        <Text size="3" emphasis="medium">Start building with Kookie UI components.</Text>
...
For larger patterns, such as code blocks, documentation layouts and marketing sections, copy a [block](/docs/blocks) into your project. Blocks are source files built on Kookie UI, not a package, so there is nothing else to install beyond the few dependencies each one lists.
