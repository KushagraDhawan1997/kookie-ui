...
Badge text always reads the strong ink for its color. Gray badges with a solid fill use step 12, so no extra prop is needed.

```tsx
<Flex gap="2">
  <Badge color="gray" variant="solid">Draft</Badge>
  <Badge color="red">Failed</Badge>
</Flex>
```
