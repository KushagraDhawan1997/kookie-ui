...
With `menuVariant="solid"`, a gray sidebar fills its active items with step 12 on its own:

```tsx
<Sidebar.Root color="gray" menuVariant="solid">
  {/* content */}
</Sidebar.Root>
```
...
- Solid gray menu items fill with step 12 automatically, and labels read the strong ink

### Removed

- `highContrast` on `Sidebar.Root` and in `BadgeConfig`. Delete it; gray and text contrast are now automatic
