...
| Prop           | Type                      | Default  | Description                                                     |
| -------------- | ------------------------- | -------- | --------------------------------------------------------------- |
| `label`        | `ReactNode`               | `'Back'` | Text for the back row in drill-down mode, usually the submenu title |
...
In drill-down mode, `open` and `defaultOpen` push the submenu onto the navigation stack, and `onOpenChange` reports every push and pop. A controlled `open` is applied when it changes: the user can still navigate in and out, and your `onOpenChange` handler is where you keep `open` in sync.

### SubTrigger

| Prop        | Type        | Description                                                     |
| ----------- | ----------- | --------------------------------------------------------------- |
| `disabled`  | `boolean`   | Prevents interaction when `true`. A disabled trigger never opens its submenu |
| `textValue` | `string`    | Text used for typeahead and for the drill-down panel name       |
...
| `virtualized`  | `boolean`                  | `false`          | Skips the ScrollArea so a `VirtualMenu` scrolls itself. Cascade mode only |
| `container`    | `HTMLElement`              | —                | Custom container for the portal                                    |
| `sideOffset`   | `number`                   | `1`              | Distance from the sub trigger in pixels                            |
| `alignOffset`  | `number`                   | Content padding  | Lines the first item up with the sub trigger                       |

`SubContent` has no `submenuBehavior`; the parent `Content` decides it. In drill-down mode the visual and positioning props do not apply, because the panel renders inline in the parent content.
...
Frosted glass effect for depth over images or dynamic backgrounds. Item text reads the strong ink by default.
...
The `label` prop on `Sub` sets the text of the back row at the top of the panel. It defaults to "Back". Screen readers hear "Back from Settings" for `label="Settings"`, and the panel is a group named after its trigger.

Only the levels you have navigated into are mounted, so a menu with many submenus opens as fast as a flat one. Arrow keys, typeahead and `Home`/`End` only reach the items on screen.
...
| `itemLabel`         | `keyof T \| (item: T) => ReactNode`       | —       | Label accessor for simple text items             |
| `renderItem`        | `React.ComponentType<RenderItemProps<T>>` | —       | Custom component for rendering items             |
| `onSelect`          | `(item: T, index: number) => void`        | —       | Callback when an item is selected                |
| `estimatedItemSize` | `number \| (index: number) => number`     | From size | Row height. Defaults to the menu item height for the current size and theme scaling (24px or 32px at 100%) |
...
| `data-index`      | `number`          | Index for event delegation                         |
| `onMouseEnter`    | `() => void`      | Hover handler                                      |
| `onClick`         | `() => void`      | Selection handler                                  |
| `onKeyDown`       | `(e) => void`     | Keyboard handler                                   |
...
| Key                     | Action                                          |
| ----------------------- | ----------------------------------------------- |
| `Space` / `Enter`       | Opens menu from trigger, selects focused item. In drill-down mode, opens the submenu or activates the back row |
| `Arrow Down`            | Opens menu from trigger, moves focus down       |
| `Arrow Up`              | Moves focus up. Set `loop` on `Content` to wrap |
| `Arrow Right`           | Opens the submenu of the focused SubTrigger and focuses its first item |
| `Arrow Left`            | Closes the submenu (cascade) or goes back one level (drill-down), returning focus to the SubTrigger |
| `Escape`                | In drill-down mode, goes back one level. At the root, closes the menu and returns focus to trigger |
...
- `aria-keyshortcuts` on items with a `shortcut`; the visual hint is hidden from screen readers so it is not read as part of the item name
- Drill-down panels are groups named after their trigger, and the back row is named "Back from …"
...
- In drill-down mode, keyboard navigation moves focus to the first item of the new panel and back to the SubTrigger on return. The scroll position of each level is restored
- A drill-down menu always reopens at its root level
...
- Drill-down SubTrigger and back row are real menu items: arrow keys, typeahead and `Home`/`End` reach them
- Drill-down focus moves into the new panel and back to the SubTrigger instead of dropping to the page
- `Escape` and `Arrow Left` go back one drill-down level; `Arrow Right` opens a submenu
- Drill-down mounts only the levels on the navigation stack instead of every submenu
- Drill-down panels nested in groups and radio groups now show
- SubTrigger forwards its props and honors `disabled` in drill-down mode
- `Sub` honors `open`, `defaultOpen` and `onOpenChange` in drill-down mode
- Switching between cascade and drill-down no longer remounts the menu
- A responsive `size` no longer places submenus at the viewport origin
- `SubContent` uses its own `size`, `variant`, `color` and `material`, and supports `virtualized`
- Menus follow a nested `Theme` `material`
- `TriggerIcon` keeps its `className`
- Label and muted text meet AA contrast
