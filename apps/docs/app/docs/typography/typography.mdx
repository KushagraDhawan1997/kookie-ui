...
Kookie UI defines the font *stacks* but does not fetch the webfonts for you. Every stack falls back to a system font, so the theme renders correctly out of the box — but Inter, JetBrains Mono and Playfair Display only appear once you load them.

Load them from your document, so the font CSS is discovered on the first parse and fetched in parallel with your stylesheet:
...
Request only the families you actually use — if you have overridden `--font-sans`, you do not need Inter at all.

If you cannot add tags to your document, import the bundled stylesheet instead. It is one line, but it costs an extra serialised round trip before first paint, because a CSS `@import` cannot be discovered until the importing stylesheet has been downloaded and parsed:
