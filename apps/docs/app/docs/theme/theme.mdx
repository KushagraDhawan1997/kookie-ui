...
Leave it unset and headings follow `fontFamily`. The typefaces behind `sans`, `mono` and `serif` come from the `--font-sans`, `--font-mono` and `--font-serif` tokens — see [Typography](/docs/typography) for how to point those at your own fonts.
