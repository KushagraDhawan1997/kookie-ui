...
Use controlled props (`open` / `state`) with their matching change handlers. Do not pass both controlled and default props.

### Controlled vs Default

- `open` + `defaultOpen` is invalid for Rail/Panel/Inspector/Bottom
- `state` + `defaultState` is invalid for Sidebar
- When controlled, always pass `onOpenChange` / `onStateChange`

### Rail + Panel Conflicts

When both Rail and Panel are controlled, Shell resolves conflicts through callbacks:

- If Panel is opened while Rail is closed, Shell calls `Rail.onOpenChange(true, { reason: 'panel' })`
- If Rail is closed while Panel is open, Shell calls `Panel.onOpenChange(false, { reason: 'left' })`
...
function App() {
  ...
  return (
    <Shell.Root>
      ...
      <Shell.Content>
        <Button onClick={() => setRailOpen(false)}>Close Rail</Button>
...
- `Shell.Rail` and `Shell.Sidebar` are `navigation` landmarks; `Shell.Inspector` and `Shell.Bottom` are `complementary`
- Every default landmark and label can be replaced by passing `role` or `aria-label`

### Keyboard Navigation

- Tab navigation through all interactive elements
- Escape key closes overlay presentations, and cancels an in-progress drag
- Arrow keys adjust a focused resize handle; `Home`/`End` jump to `minSize`/`maxSize`, `Shift` uses a larger step
- A burst of key presses is one resize: `onResizeEnd`, `onSizeChange` and persistence run when the keys stop
- Enter/Space for trigger activation
- Resize handles show a focus indicator when reached by keyboard

### Screen Reader Support

- Resize handles follow the ARIA window splitter pattern: `role="separator"`, `aria-valuenow`, `aria-valuemin`, `aria-valuemax` and `aria-valuetext`, all tracking the pane's real size
- `Shell.Trigger` exposes `aria-expanded` and points `aria-controls` at the pane it operates
...
- ARIA window splitter implementation for resize handles
- Focus management for overlay presentations
- `inset` prop for floating pane layouts with gray backdrop

### Changed

- Resize handles are `role="separator"` (the ARIA window splitter pattern) instead of `role="slider"`, and `aria-valuenow` now follows the pane's real size
- Keyboard resizing commits once per key burst rather than once per key press, so `onResizeEnd`, `onSizeChange` and size persistence run once
- `--shell-inset-gap` is declared on `.rt-ShellRoot[data-has-inset]`; override it there
- `height="full"` resolves to `100dvh` where supported, so mobile browsers no longer overshoot
